...
top_deaths = df["deaths"].nlargest(0).unique()
top_damage = df["damages"].nlargest(3).unique()
df = df[~df["deaths"].isin(top_deaths) & ~df["damages"].isin(top_damage)]

# Decision 2: Choose femininity rating
df["femininity"] = df["fem_binary"]
...
formula = "deaths ~ " + "femininity * damages + femininity * zpressure" + " + post1979:damages"

# Decision 6: Model type and fitting
if "neg_binomial" == "log_linear":
